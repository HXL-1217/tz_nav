...
- PCL
- VTK 9
- OctoMap（由 `ros-humble-octomap` 提供）
- Eigen
- MPI（`libopenmpi-dev`，仅编译期用于修复 VTK 配置）
- CMake / colcon

> 以下命令在 **arm64 与 amd64** 上完全一致，apt 会自动选取对应架构的包：

```bash
sudo apt update
sudo apt install \
  libpcl-dev libvtk9-dev libopenmpi-dev libeigen3-dev \
  ros-humble-octomap
```

> 说明：仓库不再内置预编译的 `liboctomap/liboctomath`，统一改由目标机的系统包提供，因此 arm64 / amd64 均可直接编译，无需任何架构相关的额外处理。

---

## 编译方法

在 ROS2 工作空间的 `src/` 下放置本包后，回到工作空间根目录执行：
