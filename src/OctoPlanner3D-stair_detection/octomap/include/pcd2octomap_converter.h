...
public:
  ...
  // 直接从已有 .bt 文件读回 OcTree(跳过 PCD->OctoMap 转换)。
  // 分辨率由 .bt 文件头恢复。读成后用 getOctomap() 取出。
  bool loadFromBtFile(const std::string & bt_path);
...
private:
  ...
  // int min_points_per_voxel_ = 1;     // 每个 voxel 至少多少个点才算占据
  // int min_cluster_voxels_ = 1;       // 连通 voxel 数少于该值则视为噪点

  // spiral
  // double resolution_ = 0.4;          // Octomap 分辨率，单位：米
  // int min_points_per_voxel_ = 1;     // 每个 voxel 至少多少个点才算占据
  // int min_cluster_voxels_ = 1;       // 连通 voxel 数少于该值则视为噪点
  ...
  int min_cluster_voxels_ = 5;       // 连通 voxel 数少于该值则视为噪点
  // ===============================================================
