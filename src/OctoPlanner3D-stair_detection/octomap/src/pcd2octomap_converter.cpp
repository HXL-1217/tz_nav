...
bool Pcd2OctomapConverter::loadFromBtFile(const std::string & bt_path)
{
  // readBinary 会从 .bt 文件头读取并恢复分辨率, 构造时随便给个占位分辨率即可。
  // 先确认文件存在/可读, 把"文件缺失"和"解析失败"区分开, 方便定位重建原因。
  {
    std::ifstream probe(bt_path, std::ios::binary);
    if (!probe) {
      std::cerr << "[CACHE MISS] .bt file missing or unreadable: " << bt_path << std::endl;
      return false;
    }
  }
  tree_ = std::make_shared<octomap::OcTree>(0.1);
  if (tree_->readBinary(bt_path)) {
    std::cout << "Loaded OctoMap from " << bt_path << std::endl;
    return true;
  }

  std::cerr << "[CACHE MISS] failed to parse OctoMap from " << bt_path
            << " (file exists but readBinary failed)" << std::endl;
  tree_.reset();
  return false;
}
