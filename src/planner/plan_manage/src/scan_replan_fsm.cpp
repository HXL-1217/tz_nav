...
namespace scan_planner
{
  ...
  void SCANReplanFSM::init(rclcpp::Node *node)
  {
    ...
    rviz_goal_height_override_ = load_parameter<double>(node_, "fsm.rviz_goal_height", NAN);
  ...
  void SCANReplanFSM::odometryCallback(const nav_msgs::msg::Odometry::ConstSharedPtr &msg)
  {
    ...
    {
      rviz_goal_height_ = std::isnan(rviz_goal_height_override_) ? odom_pos_(2) : rviz_goal_height_override_;
      rviz_height_ready_ = true;
      RCLCPP_INFO(node_->get_logger(), "Set RViz goal height to %.3f (%s)", rviz_goal_height_,
                  std::isnan(rviz_goal_height_override_) ? "from initial body_pose z"
                                                          : "from fsm.rviz_goal_height param");
    }
  ...
  void SCANReplanFSM::execFSMCallback()
  {
    ...
    case EXEC_TRAJ:
    {
      ...
      if (t_cur > info->duration_ - 1e-2)
      {
        ...
        // navi_mode 3 (reference path): the just-ended trajectory was only a
        // local segment toward a local target, NOT the global goal. When the
        // collision-free lookahead shrinks below thresh_replan the segment
        // becomes shorter than thresh_replan, so EXEC_TRAJ never re-enters
        // REPLAN_TRAJ and the trajectory simply expires here. Without this
        // guard the robot gives up (WAIT_TARGET) far from the goal. Instead,
        // replan from the current pose and keep creeping toward the goal.
        // (setStartStateFromOdomOrCurrentTraj falls back to odom once the
        // trajectory is expired, and replan_fail_count_/finishProcess bound
